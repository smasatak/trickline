...
- [x] タイマー付きカメラ撮影→即比較（F-14。2026-07-14実装済み）
- [x] 端末内保存・動画ライブラリ・セッション保存（D-15ローカルファースト化。2026-07-14着手）
- [ ] オフライン動作の仕上げ（骨格モデルのCDN依存を解消するか事前キャッシュする）
- [ ] 端末のフォトライブラリとの深い連携（動画の書き出し）
